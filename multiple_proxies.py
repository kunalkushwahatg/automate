...
class MultipleBrowserTest:
    ...
    async def check_and_validate_checkboxes(self):
        ...
        try:
            ...
            if missing_modules:
                self.logger.error(f"❌ CHECKBOX ERROR: Missing modules: {', '.join(missing_modules)}")
                return 'checkbox_error'
            else:
                self.logger.info(f"✅ SUCCESS: All required modules available: {', '.join(self.required_modules)}")
                return 'success'
